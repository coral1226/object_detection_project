...
def create_mobilenetv3_small_ssd_lite(num_classes, width_mult=1.0, use_batch_norm=True, onnx_compatible=False, is_test=False):
    ...
    print("ok", sum(p.numel() for p in base_net.parameters() if p.requires_grad))

    source_layer_indexes = [ 5, 11 ]
    extras = ModuleList([
        Block(3, 32, 64, 38, hswish(), None, stride=2),
        Block(3, 38, 48, 38, hswish(), None, stride=2),
        Block(3, 38, 38, 32, hswish(), None, stride=2),
        Block(3, 32, 30, 24, hswish(), None, stride=2)
    ])

    regression_headers = ModuleList([
        SeperableConv2d(in_channels=round(48 * width_mult), out_channels=6 * 4,
                        kernel_size=3, padding=1, onnx_compatible=False),
        SeperableConv2d(in_channels=48, out_channels=6 * 4, kernel_size=3, padding=1, onnx_compatible=False),
        SeperableConv2d(in_channels=48, out_channels=6 * 4, kernel_size=3, padding=1, onnx_compatible=False),
        SeperableConv2d(in_channels=32, out_channels=6 * 4, kernel_size=3, padding=1, onnx_compatible=False),
        SeperableConv2d(in_channels=32, out_channels=6 * 4, kernel_size=3, padding=1, onnx_compatible=False),
        Conv2d(in_channels=24, out_channels=6 * 4, kernel_size=1),
    ])

    classification_headers = ModuleList([
        SeperableConv2d(in_channels=round(48 * width_mult), out_channels=6 * num_classes, kernel_size=3, padding=1),
        SeperableConv2d(in_channels=48, out_channels=6 * num_classes, kernel_size=3, padding=1),
        SeperableConv2d(in_channels=48, out_channels=6 * num_classes, kernel_size=3, padding=1),
        SeperableConv2d(in_channels=32, out_channels=6 * num_classes, kernel_size=3, padding=1),
        SeperableConv2d(in_channels=32, out_channels=6 * num_classes, kernel_size=3, padding=1),
        Conv2d(in_channels=24, out_channels=6 * num_classes, kernel_size=1),
    ])
